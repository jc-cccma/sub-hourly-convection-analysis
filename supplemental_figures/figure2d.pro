;2D contour plot, dWCAPElse vs omega, and contours show total PCP
;====================================================================================
...
pchf0     = FLTARR(nlon0*nlat0*(ntim))
PCAPE0    = FLTARR(nlon0*nlat0*(ntim))
PCAPElse0 = FLTARR(nlon0*nlat0*(ntim))
OMEGA0    = FLTARR(nlon0*nlat0*(ntim))

counter0=FLTARR(1)

FOR l0=0,nlon0-1 DO BEGIN
 FOR l1=0,nlat0-1 DO BEGIN
  FOR l2=1,ntim-2 DO BEGIN
...
FOR XX=0,N_BINS-1 DO BEGIN
	FOR YY=0,N_BINS-1 DO BEGIN
		INDEX = WHERE((VAR_Y[*] GT (Y_MIN+(STEP_Y*YY))) AND (VAR_Y[*] LE (Y_MIN+(STEP_Y*(YY+1)))) AND (VAR_X[*] GT (X_MIN+(STEP_X*XX))) AND (VAR_X[*] LE (X_MIN+(STEP_X*(XX+1)))) AND (pchf0[*] GT MIN_RAIN))		
                IF (N_ELEMENTS(INDEX) GT 50.) THEN BEGIN
...
;===================================================================================
rpsopen, 'figure2d_tp.eps', /encap, xs=14, ys=10, /inches, /color

...
filename1b = '/home/rtm/GCM/CanAM4_hires/ensamble/plhf/TWP_sa_jcl_hf_1997_plhf_gs.nc'
...
; IF - detects time when convective rainfall starts and ends. Also it saves maximum convective rainfall
   IF (pchf[NLON,NLAT,nt-3] LT 0.1) AND (pchf[NLON,NLAT,nt-2] LT 0.1) AND (pchf[NLON,NLAT,nt-1] LT 0.1) AND (pchf[NLON,NLAT,nt] GT 0.1) AND $
      ...
      (pchf[NLON,NLAT,nt-10] LT 0.1) AND (pchf[NLON,NLAT,nt-9] LT 0.1) AND (pchf[NLON,NLAT,nt-8] LT 0.1) AND (pchf[NLON,NLAT,nt-7] LT 0.1) THEN BEGIN
    ...
    FOR TIME2=nt+1,nt+48 DO BEGIN
     ...
     ENDIF
     IF (pchf[NLON,NLAT,TIME2] LT 0.1) THEN GOTO, OUT
    ENDFOR ; TIME2
   ENDIF
...
FOR newtime1=0,COUNT-1 DO BEGIN
 ...
 FOR newtime2=12,70 DO BEGIN
   IF (arr_prc[newtime1, newtime2] LT 0.1) AND (end_event EQ 0) THEN BEGIN
     end_event=newtime2
...
rpsopen, 'GCM18_hires_ENSAMBLE_rtm_1_TWP_all_forcing_1mm_time_rCAPE_total_prec.eps', /encap, xs=10, ys=30, /inches, /color
